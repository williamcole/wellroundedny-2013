...
function wrny_ads( $banner = false ) {
...
function wrny_ad_markup( $id ) {
	...
	$ad_banner = get_post_meta( $ad_id, 'banner', true );
	$ad_img_size = ( $ad_banner ) ? 'full' : 'ad';
	$ad_box_id = ( $ad_banner ) ? 'banner-da' : 'da';
	$ad_html = '';
	...
	switch( $ad_type ) {
		...
		case 'Image Link' :
			...
			if( !empty( $ad_img ) ) {
				...
				$ad_img_html = wp_get_attachment_image( $ad_img_id, $ad_img_size );
			}
			$ad_link = get_post_meta( $ad_id, 'link', true );
			if( !empty( $ad_img_html ) && !empty( $ad_link ) ) {
				$ad_html .= '<div id="' . $ad_box_id . '" class="' . $ad_class . '">';
...
function wrny_banner_ad() {
	wrny_ads( $banner = true );
}
